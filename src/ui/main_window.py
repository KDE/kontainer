from PyQt6.QtWidgets import (
    QMainWindow, QListWidget, QVBoxLayout, QWidget, QMenu,
    QMessageBox, QPushButton, QHBoxLayout, QTreeWidget, QTreeWidgetItem,
    QSplitter, QTabWidget, QLabel, QLineEdit, QTextEdit,
    QToolBar, QStatusBar
)
...
class MainWindow(QMainWindow):
    ...
    def _setup_left_panel(self):
        ...
        self.container_tree.setContextMenuPolicy(
            Qt.ContextMenuPolicy.CustomContextMenu
        )
        self.container_tree.customContextMenuRequested.connect(
    ...
    def _setup_apps_tab(self):
        ...
        # Available apps
        self._setup_available_apps_tab()
        
        # Exported apps
        self._setup_exported_apps_tab()
        
        self.apps_layout.addWidget(self.apps_tabs)
        self.right_panel.addTab(self.apps_tab, "Applications")

    def _setup_available_apps_tab(self):
    ...
    def _setup_exported_apps_tab(self):
    ...
    def _setup_bottom_buttons(self):
        ...
        self.create_box_button.clicked.connect(self.show_create_box_dialog)
    ...
    def _create_toolbar(self):
        ...
        self.enter_action = QAction(
            QIcon.fromTheme("utilities-terminal"), "Enter", self
        )
        self.enter_action.triggered.connect(self.enter_current_box)
        toolbar.addAction(self.enter_action)

        self.upgrade_action = QAction(
            QIcon.fromTheme("system-upgrade"), "Upgrade", self
        )
    ...
    def load_containers(self):
        ...
        try:
            boxes = self.backend.get_all_distroboxes()
            for box in boxes:
    ...
    def show_context_menu(self, pos):
        ...
        menu.addAction(QIcon.fromTheme("utilities-terminal"), "Open Terminal", 
                      lambda: self.backend.enter_box(self.current_box))
        menu.addAction(QIcon.fromTheme("system-upgrade"), "Upgrade Box", 
                      lambda: self.backend.upgrade_box(self.current_box))
    ...
    def load_apps_for_box(self, box_name):
        try:
            ...
            available_apps = self.backend.get_apps_from_container_fs(box_name)
            if available_apps:
            ...
            exported_apps = self.backend.get_apps_in_box(box_name)
            if exported_apps:
    ...
    def export_selected_app(self):
        ...
        try:
            result = self.backend.export_app(app_name, self.current_box)
    ...
    def remove_selected_app(self):
        ...
        app_name = selected.text()
        try:
            result = self.backend.remove_exported_app(app_name, self.current_box)
            QMessageBox.information(self, "Remove App", result)
            self.load_apps_for_box(self.current_box)
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to remove app:\n{str(e)}")

    def show_create_box_dialog(self):
        from .dialogs import CreateBoxDialog
        from PyQt6.QtWidgets import QProgressDialog

        dialog = CreateBoxDialog(self.backend, self)
        if dialog.exec():
            ...
            progress.setWindowTitle("Creating DistroBox")
            progress.setWindowModality(Qt.WindowModality.WindowModal)
            progress.setCancelButton(None)  # Disable cancel button
            progress.show()

            # Use a QTimer to process events and show the dialog
            from PyQt6.QtCore import QTimer
            QTimer.singleShot(100, lambda: self._create_box_threaded(data, progress))

    def _create_box_threaded(self, data, progress):
        def worker():
            try:
                result = self.backend.create_box(
                    data['name'],
                    data['image'],
                    data['home'],
                    data['use_init'],
                ...
                self.load_containers()
                progress.close()
                QMessageBox.information(self, "Success", f"Box created:\n{result}")
            except Exception as e:
                progress.close()
    ...
    def delete_box(self, box_name):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                out = self.backend.delete_box(box_name)
    ...
    def enter_current_box(self):
        if self.current_box:
            self.backend.enter_box(self.current_box)

    def upgrade_current_box(self):
        if self.current_box:
            self.backend.upgrade_box(self.current_box)
